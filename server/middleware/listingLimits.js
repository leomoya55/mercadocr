...
const { getPlan, isOwner, PLAN_RANK } = require('../config/plans');
...
const enforceListingLimit = async (req, res, next) => {
  try {
    ...
    // Owner and Pro users have unlimited listings
    if (isOwner(req.email) || user.plan === 'pro') {
    ...
    // Over plan limit — check for single-post credits (atomic decrement)
    if (user.singlePostCredits > 0) {
      ...
      if (decremented) {
        req.usedCredit = true; // handler can log/track this if needed
        return next();
      }
    }

    // Limit reached — reject with structured payload the frontend can parse
...
module.exports = { getActiveCount, enforceListingLimit, requirePlan };
